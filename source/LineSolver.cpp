...
namespace VCLab
{
	void LineSolver::Solver(ProjectInput VCLInput, vector<Phase> SysPhases)
	{
		...
		if (VCLInput.Var == "TEMPERATURE")
		{
			...
			//delete[] LGGP.GP;
			WriteResults();
			for (i = 0;i < SysPhases.size();i++)
			{
				//delete[] SysPhases[i].GPGF;
				//delete[] SysPhases[i].GPSF;
				//delete[] SysPhases[i].GPTC;
				//delete[] SysPhases[i].GPBMAGN;
				//delete[] SysPhases[i].GPtao;
			}
		}
		else
		{
			cout << "This type of variblies: " << VCLInput.Var << " is not inplemented now!" << endl;
		}
	}
