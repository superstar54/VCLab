...
namespace VCLab
{
	using namespace std;
	//
	void Database::defineSystem(vector<string> EleList, vector<string> SelPhaList, 
		vector<string> RejPhaList, string FilTDB)
	{
		...
		getElement(EleList);
		getPhase(EleList);
		regetPhase(SelPhaList, RejPhaList);
	...
	void Database::regetPhase(vector<string> SelPhaList,
		vector<string> RejPhaList)
	{
		int i, j;
		vector<Phase> tempPhases;
		int tempPhn;
		int rejb;

		// Select
		tempPhases = SysPhases;
		SysPhases.clear();
		tempPhn = tempPhases.size();
		for (i = 0;i < tempPhn;i++)
		{
			if (SelPhaList[0] != "ALL")
			{
				for (j = 0;j < SelPhaList.size();j++)
				{
					if (tempPhases[i].name == SelPhaList[j])
					{
						SysPhases.push_back(tempPhases[i]);
						break;
					}
				}
			}
		...
		{
			rejb = 0;
			if (RejPhaList[0] != "NONE")
			{
				for (j = 0;j < RejPhaList.size();j++)
				{
					if (tempPhases[i].name == SelPhaList[j])
					{
						rejb = 1;
